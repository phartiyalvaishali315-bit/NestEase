    ...
    <div className="min-h-screen bg-gray-50">
      <nav className="bg-gradient-to-r from-green-800 to-green-600 text-white px-6 py-4 shadow-lg">
        <div className="max-w-2xl mx-auto flex items-center gap-3">
          {room ? (
            <button onClick={() => { setRoom(null); setSelect(null); }} className="bg-green-700 w-8 h-8 rounded-lg flex items-center justify-center">←</button>
          ) : (
            <button onClick={() => navigate(-1)} className="bg-green-700 w-8 h-8 rounded-lg flex items-center justify-center">←</button>
          )}
          <h1 className="text-lg font-bold">{room ? `Chat — ${selectedApp?.property_title}` : 'Messages'}</h1>
        </div>
      </nav>
        ...
        <div className="max-w-2xl mx-auto p-6 space-y-3">
          {loading ? (
            ...
            <div className="text-center py-20">
              ...
              <p className="text-gray-400 text-sm mt-1">Chat unlocks after application is approved</p>
            </div>
          ) : apps.map(app => (
            <button
              key={app.id}
            ...
            >
              ...
              <div>
                <p className="font-bold text-gray-800">{app.property_title}</p>
                <p className="text-gray-500 text-sm">Tap to open chat</p>
              </div>
        ...
        <div className="max-w-2xl mx-auto flex flex-col" style={{ height: 'calc(100vh - 72px)' }}>
          ...
          <div className="bg-white border-t p-4 flex gap-3">
            <input
              ...
              className="flex-1 border rounded-xl px-4 py-2 outline-none focus:border-green-500 text-sm"
            />
            <button
              onClick={handleSend}
              className="bg-green-700 text-white px-4 py-2 rounded-xl font-bold text-sm hover:bg-green-800 transition"
            >Send</button>
          </div>
        </div>
      )}
